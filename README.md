...
This project includes Gradle configuration files (`build.gradle`, `settings.gradle`, and Gradle Wrapper files).

1.  **Clone this repository.**
2.  **Import into Android Studio:**
    *   Open Android Studio.
    *   Select "Open" (or "Import Project" if you don't have a project open).
    *   Navigate to the cloned repository's root directory and select it.
    *   Android Studio should automatically detect it as a Gradle project and sync it.
3.  **Connect an Android emulator or a physical device** (with USB debugging enabled).
4.  Once the Gradle sync is complete, select the `app` configuration from the run configurations dropdown.
